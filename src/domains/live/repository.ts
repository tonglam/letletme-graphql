...
const redisKey = {
	eventLive: (season: string, eventId: number): string =>
...
export const mapSyncJobLiveRow = (raw: unknown): LivePerformance | null => {
	...
	return {
		...
		starts: startsValue,
		defensiveContribution: asNumber(
			row.defensiveContribution ?? row.defensive_contribution,
		),
		expectedGoals: asString(row.expectedGoals ?? row.expected_goals) ?? null,
		expectedAssists:
			asString(row.expectedAssists ?? row.expected_assists) ?? null,
		expectedGoalInvolvements:
			asString(
				row.expectedGoalInvolvements ?? row.expected_goal_involvements,
			) ?? null,
		expectedGoalsConceded:
			asString(row.expectedGoalsConceded ?? row.expected_goals_conceded) ??
			null,
...
const mapLiveExplainStats = (
	statsValue: DbLiveExplainStats | null,
): LiveExplainStats => {
	const stats: DbLiveExplainStats = statsValue ?? {};
	return {
		minutes: parseIntegerValue(pickRecordValue(stats, "minutes")),
		goalsScored: parseIntegerValue(
			pickRecordValue(stats, "goals_scored", "goalsScored"),
		),
		assists: parseIntegerValue(pickRecordValue(stats, "assists")),
		cleanSheets: parseIntegerValue(
			pickRecordValue(stats, "clean_sheets", "cleanSheets"),
		),
		goalsConceded: parseIntegerValue(
			pickRecordValue(stats, "goals_conceded", "goalsConceded"),
		),
		ownGoals: parseIntegerValue(
			pickRecordValue(stats, "own_goals", "ownGoals"),
		),
		penaltiesSaved: parseIntegerValue(
			pickRecordValue(stats, "penalties_saved", "penaltiesSaved"),
		),
		penaltiesMissed: parseIntegerValue(
			pickRecordValue(stats, "penalties_missed", "penaltiesMissed"),
		),
		yellowCards: parseIntegerValue(
			pickRecordValue(stats, "yellow_cards", "yellowCards"),
		),
		redCards: parseIntegerValue(
			pickRecordValue(stats, "red_cards", "redCards"),
		),
		...
		ictIndex: parseNumericValue(
			pickRecordValue(stats, "ict_index", "ictIndex"),
		),
		...
		expectedGoals: parseNumericValue(
			pickRecordValue(stats, "expected_goals", "expectedGoals"),
		),
		...
		totalPoints: parseIntegerValue(
			pickRecordValue(stats, "total_points", "totalPoints"),
		),
		inDreamTeam: parseBooleanValue(
			pickRecordValue(stats, "in_dreamteam", "inDreamTeam"),
		),
	};
};
...
const mapLiveExplainBreakdownStat = (
	...
	const identifier =
		typeof identifierValue === "string" ? identifierValue : null;
...
const mapLiveExplainBreakdown = (
	...
	for (const entry of explainValue) {
		...
		const fixtureId = parseIntegerValue(
			pickRecordValue(entry, "fixture", "fixtureId"),
		);
		if (fixtureId === null) {
			continue;
		}
...
const mapBreakdownFromEventLiveRow = (
...
async function fetchEventLiveExplainFromSupabase(
	...
	const col = await detectExplainElementCol(context);
	const { data, error } = await context.supabase
	...
	if (error) {
		context.logger.error(
			{ err: error, eventId, elementId },
			"event_live_explains query failed",
		);
...
type ExplainElementCol = "element_id" | "element";
let explainElementCol: ExplainElementCol | null = null;
let explainElementColPromise: Promise<ExplainElementCol> | null = null;

const detectExplainElementCol = (
	context: GraphQLContext,
): Promise<ExplainElementCol> => {
	if (explainElementCol !== null) return Promise.resolve(explainElementCol);
	if (explainElementColPromise) return explainElementColPromise;

	explainElementColPromise = (async (): Promise<ExplainElementCol> => {
		const { error } = await context.supabase
			.from("event_live_explains")
			.select("element_id")
			.limit(0);
		if (!error) {
			explainElementCol = "element_id";
			return "element_id";
		}
		explainElementCol = "element";
		return "element";
	})();

	return explainElementColPromise;
};
...
export const liveRepository: LiveRepository = {
	async getAllLivePerformances(
		...
		const shapedCacheKey = `live:all:${eventId}`;
		const shapedCached = await context.redis.get(shapedCacheKey);
		if (shapedCached) {
			const arr = JSON.parse(shapedCached) as LivePerformance[];
			return new Map(arr.map((p) => [p.playerId, p]));
		}
		...
		const performances =
			fromRedis ??
			new Map(
				(await fetchAllLivePerformanceFromDb(context, eventId)).map((p) => [
					p.playerId,
					p,
				]),
			);
	...
	async getLiveScores(
		...
		const allPerformances = await this.getAllLivePerformances(
			context,
			targetEventId,
		);
		...
		if (minTotalPoints !== undefined && minTotalPoints !== null) {
			results = results.filter((p) => p.totalPoints >= minTotalPoints);
		}
		const maxTotalPoints = filter?.maxTotalPoints;
		if (maxTotalPoints !== undefined && maxTotalPoints !== null) {
			results = results.filter((p) => p.totalPoints <= maxTotalPoints);
		}
	...
	async getPlayerLive(
		...
		const performances = await this.getLivePerformancesByPlayerIds(
			context,
			targetEventId,
			[playerId],
		);
	...
	async getEventLiveExplain(
		...
		if (cachedRaw !== null) {
			...
			return JSON.parse(cachedRaw) as LiveExplain;
		}
		...
		let breakdown: LiveExplainBreakdown[] = elRow
			? mapBreakdownFromEventLiveRow(elRow)
			: [];
	...
	async getLivePerformancesByPlayerIds(
		...
		try {
			...
			context.logger.warn(
				{ err, hashKey },
				"EventLive HMGET failed, falling back to DB",
			);
			return fetchLivePerformanceFromDbByEventsAndPlayerIds(
				context,
				[eventId],
				uniqueIds,
			);
