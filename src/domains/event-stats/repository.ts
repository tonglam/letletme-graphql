...
async function getPlayerAndTeamMaps(
	...
	const neededTeamIds = new Set(
		[...filteredPlayerMap.values()].map((p) => p.team_id),
	);
...
async function getTournamentInfo(
	...
	if (error || !data?.[0]) {
		context.logger.error(
			...
			"Failed to fetch tournament info",
		);
		return null;
	}
...
async function getTournamentEntryIds(
	...
	const entryIds = ((data as { entry_id: number }[] | null) ?? []).map(
		(r) => r.entry_id,
	);
	await context.redis.set(
		cacheKey,
...
export const eventStatsRepository: EventStatsRepository = {
	async getTournamentSelectionStats(
		...
		const { playerMap, teamMap } = await getPlayerAndTeamMaps(
			context,
			allPlayerIds,
		);

		const computeEoPercent = (
			playerId: number,
			selectedPct: number,
		): number => {
			const captainCount = captainCounts.get(playerId) ?? 0;
			const captainPct =
				effectiveTotal > 0 ? (captainCount / effectiveTotal) * 100 : 0;
		...
		const buildSelectionPlayer = (
			...
			const selectedPct =
				effectiveTotal > 0 ? (pickCount / effectiveTotal) * 100 : 0;
			return {
				id: player.id,
				...
				eoByPercent:
					Math.round(computeEoPercent(playerId, selectedPct) * 100) / 100,
			};
		};
		...
		const buildCaptainPlayer = (
			...
			const team = teamMap.get(player.team_id);
			const rolePct =
				effectiveTotal > 0 ? (roleCount / effectiveTotal) * 100 : 0;
		...
		const viceCaptainSelect: CaptainStatPlayer[] = [
			...viceCaptainCounts.entries(),
		]
			.sort((a, b) => b[1] - a[1])
			.slice(0, safeLimit)
			.map(([playerId, count]) => buildCaptainPlayer(playerId, count))
		...
		const mostTransferOut: TransferStatPlayer[] = [
			...transferOutCounts.entries(),
		]
			.sort((a, b) => b[1] - a[1])
			.slice(0, safeLimit)
			.map(([playerId, count]) => buildTransferPlayer(playerId, count))
