...
// Official H2H is one bounded publication containing the schedule, per-match
// live state, and the independent standings overlay. Keep its wider document
// allowance scoped to the exact unaliased root; complexity, depth, aliases,
// root count, and publication-size limits remain unchanged.
const TOURNAMENT_OFFICIAL_H2H_MAX_AST_NODES = 240;
...
const activeDeprecatedTelemetrySelections = (
	...
	const collectSelectionOccurrenceOwners = (
		...
		for (const selection of selectionSet.selections) {
			...
			for (const owner of collectFragmentOccurrenceOwners(
	...
	const inspect = (
		...
		for (const selection of selectionSet.selections) {
			...
			if (selection.kind === Kind.INLINE_FRAGMENT) {
				const directiveOwners = new Set<string>();
				for (const branchPath of branchPathsFor(selection, pathPrefix)) {
					for (const owner of collectSelectionOccurrenceOwners(
						selection.selectionSet,
						branchPath
...
export const validateGraphQLPayloadLimits = (
	...
	const usesTournamentOfficialH2H =
		onlyReachableDefinitions &&
		responseKeys.size === 1 &&
		rootNames.length > 0 &&
		rootNames.every(
