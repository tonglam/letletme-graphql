...
2. GraphQL `#193` tested code exact head 是
	`29372f72a4cc6b1dea62b028db944a439b447f04`，verify 与 database-contract CI
	已通过；该 head 包含 H2H 结果、POINTS tournament score 和累计成本的
	fail-closed 校验，并修复了此前针对 `4dca17f…` exact-head review 返回的 5 个 P2、
	随后发现的 1 个 P1 和 1 个 P2，以及最终 review 发现的 3 个 P2。当前报告/artefact
	位于该 tested code head 之后的 docs commit；其 exact artifact head 必须单独取得
...
| GraphQL | PR `#193` tested code head `29372f72a4cc6b1dea62b028db944a439b447f04`；focused 87 / 0 fail；完整 1050 pass、7 skip、0 fail、1 snapshot、3511 expect；typecheck、lint、format、layers、docs、deprecation、Bun build；CI verify/database-contract `33413505953`、`33413511341` 均通过 | 此前 `4dca17f…` exact-head review 的 5 个 P2、随后 1 个 P1/1 个 P2、以及最终 review 的 3 个 P2 已修复；当前报告/artefact 位于其后 docs commit，仍待该 exact artifact head review；不得把 CI green 当作 clean review |
...
| GraphQL schema/read path | CI/local gate 通过，待最终 exact-head review | tested code head `29372f72a4cc6b1dea62b028db944a439b447f04`；focused 87、完整 1050、CI 4 项通过；此前 `4dca17f…` exact-head review 的 5 个 P2、随后 1 个 P1/1 个 P2、以及最终 review 的 3 个 P2 已修复；当前报告/artefact 位于其后 docs commit，必须再取得 clean signal |
