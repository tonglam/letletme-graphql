...
2. GraphQL `#193` 当前代码 exact head 是
   `61403482fb76110a1ad8a79bbdc78118ca8c768e`，verify 与 database-contract CI
   已通过；该 head 包含 H2H 结果、POINTS tournament score 和累计成本的
   fail-closed 校验，并修复了此前针对 `4dca17f…` exact-head review 返回的 5 个 P2，
   以及随后发现的 1 个 P1 和 1 个 P2。文档更新后仍必须针对承载最终 artifact 的
   PR head 重新取得 clean signal，不能把 CI green 当作 review clean。Web/Mini 仍
...
| GraphQL | PR `#193` final docs-inclusive head `61403482fb76110a1ad8a79bbdc78118ca8c768e`；focused 84 / 0 fail；完整 1047 pass、7 skip、0 fail、1 snapshot、3498 expect；typecheck、lint、format、layers、docs、deprecation、Bun build；CI verify/database-contract `33410710886`、`33410715786` 均通过 | 此前 `4dca17f…` exact-head review 的 5 个 P2 及随后发现的 1 个 P1、1 个 P2 已修复，待最终 exact-head review；不得把 CI green 当作 clean review |
...
| GraphQL schema/read path | CI/local gate 通过，待最终 exact-head review | final docs-inclusive head `61403482fb76110a1ad8a79bbdc78118ca8c768e`；focused 84、完整 1047、CI 4 项通过；此前 `4dca17f…` exact-head review 的 5 个 P2 及随后发现的 1 个 P1、1 个 P2 已修复，最终 artifact 承载 commit 必须再取得 clean signal |
