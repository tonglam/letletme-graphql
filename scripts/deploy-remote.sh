...
for attempt in $(seq 1 "$PUBLIC_HEALTH_ATTEMPTS"); do
  if ! public_health=$(curl --fail --silent --show-error --max-time 5 "$public_health_url"); then
    echo "public GraphQL health probe failed after switching to $inactive_slot; rolling back" >&2
    if ! rollback_switch; then
      echo "public probe failed and rollback could not be verified" >&2
    fi
    exit 1
  fi
  public_identity="invalid"
  if ! public_identity=$(jq -r '
    if (type != "object") or (.status != "ok") then "invalid"
    elif (has("deploySha") | not) then "legacy"
    elif (.deploySha | type) == "string" and (.deploySha | length) > 0 then .deploySha
    else "invalid"
    end
  ' <<<"$public_health"); then
    public_identity="invalid"
  fi
  if [ "$public_identity" = "$DEPLOY_SHA" ]; then
    public_health_ready=true
    break
  fi
  case "$public_identity" in
    legacy|"$old_slot_deploy_sha")
      echo "public GraphQL health has not converged to $DEPLOY_SHA on attempt $attempt" >&2
      ;;
    *)
      echo "public GraphQL health returned an unexpected deployment identity; rolling back" >&2
      if ! rollback_switch; then
