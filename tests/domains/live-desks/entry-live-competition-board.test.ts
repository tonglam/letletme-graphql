...
const score = (input: Partial<LiveManagerScore> = {}): LiveManagerScore => ({
	...
	provenance: {
		...
		liveRevision: "1",
		liveCheckedAt: "2026-08-23T00:00:00.000Z",
		picksRevision: "picks-1",
		picksCheckedAt: "2026-08-23T00:00:00.000Z",
		...
		rankCheckedAt: "2026-08-23T00:00:00.000Z",
	},
	...input,
});
...
const liveRow = (input: {
	...
	const managerScore = score({
		...
		source: input.source ?? "FPL_EVENT_LIVE",
...
describe("entry live competition board filtering and paging", () => {
	...
	it("keeps bounded and full-field board projections in separate caches", () => {
		const context = {
			dataRevision: "core-1",
			currentSeason: { seasonCode: "2627" },
		} as GraphQLContext;
	...
	it("keeps event-team filter projections separate from unfiltered caches", () => {
		const context = {
			dataRevision: "core-1",
			currentSeason: { seasonCode: "2627" },
		} as GraphQLContext;
		...
		expect(
			entryLiveCompetitionBoardCacheKey(context, {
				...identity,
				requireEventTeamIds: false,
			})
		).not.toBe(
			entryLiveCompetitionBoardCacheKey(context, {
				...identity,
				requireEventTeamIds: true,
			})
		);
	});
