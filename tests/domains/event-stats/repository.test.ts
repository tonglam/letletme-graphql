...
describe("eventStatsRepository.getTournamentSelectionStats", () => {
	...
	it("returns cached result on cache hit", async () => {
		...
		const context = buildContext({
			redisStrings: {
				[`tournament-selection-stats:${TOURNAMENT_ID}:${EVENT_ID}:${LIMIT}`]:
	...
	it("caches the result after computing", async () => {
		...
		const cached = JSON.parse(
			strings.get(cacheKey) ?? "{}",
		) as TournamentSelectionStats;
	...
	it("uses player/team cache on second call", async () => {
		...
		await eventStatsRepository.getTournamentSelectionStats(
			context,
			TOURNAMENT_ID,
			EVENT_ID,
			LIMIT,
	...
	it("clamps limit to valid range", async () => {
		...
		const mainKey = keys.find(
			(k) => k === `tournament-selection-stats:${TOURNAMENT_ID}:${EVENT_ID}:1`,
		);
	...
	it("uses player/team lookup from Redis hash when available", async () => {
		...
		const context = {
			redis: {
				get: async (key: string): Promise<string | null> => {
					if (
						key.startsWith("tournament-selection-stats:") &&
						!key.includes("captain-counts")
					)
						return null;
					if (key.startsWith("tournament:info:")) return null;
					if (key.startsWith("tournaments:entry-ids:")) return null;
					if (key.startsWith("tournament-selection-stats:captain-counts:"))
						return null;
					return null;
				},
				set: async (
