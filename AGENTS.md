...
- Data owns `fpl`, `competition`, `understat`, `bridge`, `reporting`, `content`, `ops`, Data publications, and business migrations. Web owns authentication, sessions, and `bauth`. GraphQL owns authorization, query shaping, read-only access, root-field policy, and `llm:gql:*` query/security state.
...
- Use `$letletme-graphql-read-path` for ingress, auth, schema/domain, read-model, publication/fallback, cache, or rate-limit work. Use the global `$letletme-stack-audit` when a behavior or contract crosses Data/Web/Mini/Ops, and the global `$letletme-release-acceptance` for an authorized end-to-end release.
